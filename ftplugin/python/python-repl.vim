...
function! s:REPLDebugRunAsync() abort
    let l:code = ["wait repl#Strip(repl#GetTerminalLine()) == 'ipdb>'", 'call term_sendkeys("' . repl#GetConsoleName() . '", "c\n")', "sleep 20ms", "wait repl#Strip(repl#GetTerminalLine()) == 'ipdb>'", 'call g:REPLDebugMoveCursor()']
...
function! s:REPLDebugRun() abort
    if repl#REPLIsVisible()
        if repl#Strip(repl#GetTerminalLine()) != 'ipdb>'
            call term_sendkeys(repl#GetConsoleName(), "\n")
...
function! g:REPLDebugMoveCursor() abort
    for l:currentlinenumber in range(repl#GetCurrentLineNumber(), 1, -1)
        let l:t = term_getline(repl#GetConsoleName(), l:currentlinenumber)
        if stridx(l:t, '>') == 0
            let l:t = l:t[2:]
            let l:i = stridx(l:t, '(')
            ...
            while l:i == -1
                let l:j = l:j + 1
                let l:nextline = term_getline(repl#GetConsoleName(), l:j)
...
function! s:REPLDebugN() abort
    ...
    let l:code = ['if repl#Strip(repl#GetTerminalLine()) != "ipdb>"', 'call term_sendkeys("' . repl#GetConsoleName() . '", "\n")', 'endif', 'call term_sendkeys("' . repl#GetConsoleName() . '", "n\n")', 'sleep 20ms', 'wait repl#Strip(repl#GetTerminalLine()) == "ipdb>"', 'call g:REPLDebugMoveCursor()']
...
function! s:REPLDebugIPDB() abort
	if repl#REPLIsVisible()
		...
		let g:repl_target_f = @%
        call repl#REPLOpen('python -m ipdb '.expand('%'))
	endif
	if g:repl_stayatrepl_when_open == 0
		exe bufwinnr(g:repl_target_n) . 'wincmd w'
	endif
    call term_wait(repl#GetConsoleName(), 20)
    let l:n = 0
    while l:n < 1000
